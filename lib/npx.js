...
function animation (buffer, delay) {
  // Return a function that returns a promise to make api cleaner
  return function(){
    var animation = Q.defer()
...
Npx.prototype.walk = function (colorArray){
  // var frames = new Array(this.totalPixels+1);
  ...
  for (var i = 0; i < this.totalPixels+1; i++) {
    ...
    this.enqueue(animation(frame,10));
    // frames[i] = frame;
  }
  ...
  //   this.queue.push(animation(frame));
  // }
  var start = new Date();
  return this.run()
             .then(function(){
